...
def gerar_dataset() -> dict[str, bytes]:
    ...
    for nome, dados in [
        ("dados/repetitivo.txt", repetitivo),
        ("dados/aleatorio.bin", aleatorio),
    ]:
...
def fmt_tempo(seg: float) -> str:
    if seg < 1e-3:
        return f"{seg * 1e6:8.2f} µs"
    if seg < 1.0:
        return f"{seg * 1e3:8.2f} ms"
    return f"{seg:8.2f} s "


def fmt_velocidade(mbps: float, baseline: bool) -> str:
    if baseline:
        return f"{COR['CINZA']}     ∞ MB/s{COR['RST']}"
    if mbps > 1000:
        return f"{COR['AM_NEG']}{mbps:8.1f} MB/s{COR['RST']}"
...
def benchmark(nome_arq: str, dados: bytes) -> list[dict]:
    ...
    print(f"\n{'═' * 100}")
    print(
        f"  {NEGRITO}📂 {nome_arq}  ({tam_mb:.1f} MiB)  SHA-256: {hash_ref[:16]}…{RESETAR_COR}"
    )
    print(f"{'═' * 100}")
    print(
        f"  {'Algoritmo':<10} │ {'Taxa':>12}  {'Laudo':<28} │ {'Compressão':>14} │ {'Descompressão':>14} │ {'CPU':>12}"
    )
    print(f"{'─' * 100}")
    ...
    for nome, comprimir, descomprimir, cor in MOTORES:
        ...
        taxa_mbps_compressao = (
            tam_mb / mediana_tempo_comp if mediana_tempo_comp > 0 else float("inf")
        )
        taxa_mbps_descompressao = (
            tam_mb / mediana_tempo_decomp if mediana_tempo_decomp > 0 else float("inf")
        )
        taxa = tam_orig / tam_final if tam_final > 0 else 1.0

        print(
            f"  {cor}{nome:<10}{RESETAR_COR} │ {COR['AMAR']}{taxa:10.2f}:1{RESETAR_COR}  {laudo(tam_orig, tam_final)} "
        ...
        resultados.append(
            dict(
                algoritmo=nome,
                arquivo=nome_arq,
                tamanho_original=tam_orig,
                tamanho_final=tam_final,
                taxa=round(taxa, 4),
                throughput_mbps=round(taxa_mbps_compressao, 2),
...
def resumo(resultados: list[dict], nome_arq: str) -> None:
    ...
    score = lambda r: (
        r["taxa"] * math.log10(r["throughput_mbps"]) if r["throughput_mbps"] > 0 else 0
    )
    ...
    print(
        f"  │ 👑 Menor tamanho:    {COR['VERDE']}{menor['algoritmo']:<6}{RESETAR_COR}  (taxa {menor['taxa']:.2f}:1)"
    )
    print(
        f"  │ ⚡ Mais rápido:      {COR['AMAR']}{rapido['algoritmo']:<6}{RESETAR_COR}  ({rapido['throughput_mbps']:.1f} MB/s)"
    )
    print(
        f"  │ ⚖️  Melhor equilíbrio: {COR['CIANO']}{melhor_equilibrio['algoritmo']:<6}{RESETAR_COR}  (score {score(melhor_equilibrio):.2f})"
    )
    print(
        f"  {NEGRITO}└──────────────────────────────────────────────────┘{RESETAR_COR}"
    )
...
def main() -> None:
    ...
    print(
        f"  {NEGRITO}BENCHMARK DE COMPRESSÃO — Trade-off: Throughput (CPU) vs Taxa de Compressão (Espaço){RESETAR_COR}"
    )
    print(
        f"  Repetições: {REPETICOES}  |  Métrica: Mediana  |  Timer: process_time()  |  GC: isolado"
    )
