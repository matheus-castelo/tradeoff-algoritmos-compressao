...
def gerar_graficos():
    ...
    df_rep = df_algos[df_algos["arquivo"] == "repetitivo.txt"].sort_values(
    ...
    df_ale = df_algos[df_algos["arquivo"] == "aleatorio.bin"].sort_values(
        "taxa", ascending=False
    )
    ...
    # ---------------------------------------------------------
    df_vel = (
        df_algos.groupby("algoritmo")["throughput_mbps"]
        .mean()
        .reset_index()
        .sort_values("throughput_mbps", ascending=False)
    )
    ...
    # ---------------------------------------------------------
    df_dvel = (
        df_algos.groupby("algoritmo")["throughput_decomp_mbps"]
        .mean()
        .reset_index()
        .sort_values("throughput_decomp_mbps", ascending=False)
    )
    plt.figure(figsize=(10, 6))
    sns.barplot(
        data=df_dvel, x="throughput_decomp_mbps", y="algoritmo", color="mediumpurple"
    )
    ...
    # ---------------------------------------------------------
    df_rank = (
        df_algos.groupby("algoritmo")
        .agg({"taxa": "mean", "throughput_mbps": "mean"})
        .reset_index()
    )
